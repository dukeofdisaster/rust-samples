...
///
/// Provides a get() fn to return values associated with the specified key.
/// Is also unified with other potential implementations
///
...
///
/// Rust requires its own impl block for each trait
/// 
...
impl ConfigReader for KeyValueConfigService { 
    fn read(&self, from: &mut impl Read) -> std::io::Result<Config> {
        ...
        // chain iterators together and collect results
        let values: Vec<(String, String)> = buffer
            ...
            .collect(); // transform it into a ector
...
#[cfg(test)]
mod tests {
    // need access to the stuff we declared above
    use super::*;
    use std::io::Cursor;
    ...
    fn keyvalueconfigservice_write_config() { 
        let config = Config::new(vec![("hlo".to_string(), "wrld".to_string())]);

        let service = KeyValueConfigService::new();
        let mut target = vec![];
        assert!(service.write(config, &mut target).is_ok());

        assert_eq!(String::from_utf8(target).unwrap(), "hlo=wrld\n".to_string());
    }
    ...
    fn keyvalueconfigservice_read_config() { 
        ...
        let config = service.read(&mut Cursor::new(readable))
            .expect("couldn't read from vector");
